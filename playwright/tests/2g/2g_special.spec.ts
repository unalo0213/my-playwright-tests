...
test("2G-特殊還車", async ({ page }) => {
  ...
  await page
    .getByRole("searchbox", { name: "承租人或手機號碼" })
    .fill(config.phone);
  await page.getByRole("button", { name: "搜尋" }).click();
  const page1Promise = page.waitForEvent("popup");
  await page
    .locator(
      "button.p-1.rounded-lg.flex.justify-center.items-center.text-white.bg-teal-default"
    )
    .first()
    .click();
  ...
  await page1
    .locator(
      "div:nth-child(3) > div:nth-child(2) > div > .flex-1 > div:nth-child(2) > .w-full.relative > .px-4"
    )
    .click();
  await page1.getByRole("button", { name: "現在時間" }).click();
  await page1
    .locator(
      "div:nth-child(5) > div > div:nth-child(2) > .w-full.relative > .px-4"
    )
    .first()
    .click();
  const value = await page1
    .locator("p")
    .filter({ hasText: "車機里程：" })
    .textContent();
  ...
  await page1
    .locator(
      "div:nth-child(6) > div:nth-child(2) > .relative > #id-undefined > .css-n73go2-control > .css-hlgwow > .css-19bb58m"
    )
    .click();
